...
def optimal_control_casadi(x,u_des,cp,vel,R0):
    
    # Safety barrier 
    h = (x[0]-cp[0])**2 + (x[1]-cp[1])**2 - R0**2
    h_dot = 2*(x[0]-cp[0])*vel*cos(x[2]) + 2*(x[1]-cp[1])*vel*sin(x[2])

    # Lie Derivatives
    L2fh = 2*(vel**2) + k2*(h_dot + k1*h)
    LgLfh = -2*(x[0]-cp[0])*vel*sin(x[2]) + 2*(x[1]-cp[1])*vel*cos(x[2])
...
def plot(ax,traj,l,details,arg):
    ...
    ax.plot(traj[:,0:1],traj[:,1:2],'-',color='red',linewidth=1.5,label='CBF-PID Trajectory') 
...
def main(exp,data):
    ...
    while not goal_reached:
        ...
        # U desired calculation
        u_des,new_E,new_old_e = predict_PID(x,y,the,[xg,yg],E,old_e)
        E = new_E
        old_e = new_old_e

        # CBF constrained optimal u
        omega = optimal_control_casadi([x,y,the],u_des,cp,vel,bt)
...
if __name__ == '__main__':
    ...
    for _ in range(0,N):
        ...
        if _ == 0:
            np.save(f'Trajectories/Unicycle_u_des_HOCBF_CBF_PID_trajectory.npy',lists[0])
            np.save(f'Plots/Unicycle_u_des_HOCBF_control_inputs.npy',lists[1])  
    ...
    traje = np.load(f'Trajectories/Unicycle_u_des_HOCBF_CBF_PID_trajectory.npy')
